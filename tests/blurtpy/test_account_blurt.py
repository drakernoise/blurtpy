...
class Testcases(unittest.TestCase):
    ...
    def test_history(self):
        ...
        h_list = []
        for h in account.history(stop=5, use_block_num=False, batch_size=10, raw_output=True):
            h_list.append(h)
        # self.assertEqual(h_list[0][0], zero_element)
        self.assertEqual(h_list[-1][0], 5)
        self.assertEqual(h_list[0][1]['block'], h_all_raw[-1][1]['block'])
        self.assertEqual(h_list[-1][1]['block'], h_all_raw[-6 + zero_element][1]['block'])
        h_list = []
        ...
        self.assertEqual(h_list[-1][0], 9)
        self.assertEqual(h_list[0][1]['block'], h_all_raw[-1 + zero_element][1]['block'])
        self.assertEqual(h_list[-1][1]['block'], h_all_raw[-10 + zero_element][1]['block'])
        h_list = []
        ...
        self.assertEqual(h_list[0][1]['block'], h_all_raw[-10 + zero_element][1]['block'])
        self.assertEqual(h_list[-1][1]['block'], h_all_raw[-1 + zero_element][1]['block'])
        h_list = []
    ...
    def test_account_props(self):
        ...
        vv = account.get_voting_value_Blurt()
...
######## This has no sense on Blurt blockchain ########
...
####### Has no sense in Blurt
    ...
    def test_json_export(self):
        ...
        exclude_list = ['owner_challenged', 'average_bandwidth']  # ['json_metadata', 'reputation', 'active_votes', 'savings_BLURT_seconds']
        for k in keys:
    ...
    def test_estimate_virtual_op_num(self):
        ...
        try:
            # 1. Obtener las propiedades globales para el último número de bloque
            props = stm.get_dynamic_global_properties()
            latest_block_num = props["head_block_number"]
            
            # 2. Usar un número de bloque antiguo pero estable (ej. 100,000 bloques atrás)
            # Esto asegura que el bloque esté disponible en el historial de cualquier nodo.
    ...
    def test_get_vote_pct_for_vote_value(self):
        ...
        for vote_pwr in range(5, 100, 5):
            self.assertTrue(9900 <= account.get_vote_pct_for_vote_value(account.get_voting_value_Blurt(voting_power=vote_pwr), voting_power=vote_pwr) <= 11000)

    def test_extract_account_name(self):
        stm = self.bts
        account = Account("draktest", blurt_instance=stm)
        self.assertEqual(extract_account_name(account), "draktest")
        self.assertEqual(extract_account_name("draktest"), "draktest")
        self.assertEqual(extract_account_name({"name": "draktest"}), "draktest")
