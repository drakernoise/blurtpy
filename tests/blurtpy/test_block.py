...
from blurtpy.block import Block, BlockHeader
...
class Testcases(unittest.TestCase):
    ...
    def test_block(self):
        ...
        block = Block(test_block_id, blurt_instance=bts)
        ...
        block2 = Block(test_block_id + 1, blurt_instance=bts)
        self.assertTrue(block2.time() > block.time())
        with self.assertRaises(
            exceptions.BlockDoesNotExistsException
        ):
            Block(0, blurt_instance=bts)

    def test_block_only_ops(self):
        bts = self.bts
        test_block_id = self.test_block_id
        block = Block(test_block_id, only_ops=True, blurt_instance=bts)
        self.assertEqual(block.identifier, test_block_id)
        self.assertTrue(isinstance(block.time(), datetime))
        self.assertTrue(isinstance(block, dict))
    ...
    def test_block_header(self):
        bts = self.bts
        test_block_id = self.test_block_id
        block = BlockHeader(test_block_id, blurt_instance=bts)
        self.assertEqual(block.identifier, test_block_id)
        self.assertTrue(isinstance(block.time(), datetime))
        self.assertTrue(isinstance(block, dict))

        block2 = BlockHeader(test_block_id + 1, blurt_instance=bts)
        self.assertTrue(block2.time() > block.time())
        with self.assertRaises(
            exceptions.BlockDoesNotExistsException
        ):
            BlockHeader(0, blurt_instance=bts)
    ...
    def test_export(self):
        ...
        b = Block(block_num, blurt_instance=bts)
        ...
        b = BlockHeader(block_num, blurt_instance=bts)
        keys = list(block.keys())
        json_content = b.json()
