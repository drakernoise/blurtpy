...
from blurtpy.cli import cli, balance
from blurtpy.instance import set_shared_blurt_instance, shared_blurt_instance
from blurtbase.operationids import getOperationNameForId
from blurtpy.utils import import_pubkeys
...
class Testcases(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        result = runner.invoke(cli, ['-o', 'set', 'default_account', 'blurtpy'])
        if result.exit_code != 0:
            raise AssertionError(str(result))
    ...
    def tearDownClass(cls):
        ...
        runner = CliRunner()
        result = runner.invoke(cli, ['updatenodes', '--blurt'])

    def test_balance(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['balance', 'blurtpybot', 'blurtpypy'])
        self.assertEqual(result.exit_code, 0)

    def test_interest(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['-dx', 'interest', 'blurtpybot', 'blurtpypy'])
        self.assertEqual(result.exit_code, 0)
    ...
    def test_changerecovery(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['-dx', 'changerecovery', '-a', 'blurtpybot', 'holger80'], input=wif + "\nexit\n")
        self.assertEqual(result.exit_code, 0)
    ...
    def test_info(self):
        ...
        self.assertEqual(result.exit_code, 0)
        result = runner.invoke(cli, ['info', 'blurtpybot'])
        self.assertEqual(result.exit_code, 0)
    ...
    def test_upvote(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['-dx', 'upvote', '@blurtit/firstpost'], input="test\n")
        self.assertEqual(result.exit_code, 0)
        result = runner.invoke(cli, ['-dx', 'upvote', '--weight', '100', '@blurtit/firstpost'], input="test\n")
        self.assertEqual(result.exit_code, 0)

    def test_downvote(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['-dx', 'downvote', '--weight', '100', '@blurtit/firstpost'], input="test\n")
        self.assertEqual(result.exit_code, 0)

    def test_download(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['-dx', 'download', '-a', 'blurtit', 'firstpost'])
        self.assertEqual(result.exit_code, 0)
        result = runner.invoke(cli, ['-dx', 'download', '@blurtit/firstpost'])
        self.assertEqual(result.exit_code, 0)

    def test_transfer(self):
        stm = shared_blurt_instance()
        runner = CliRunner()
        result = runner.invoke(cli, ['-dx', 'transfer', 'blurtpybot', '1', stm.backed_token_symbol, 'test'], input="test\n")
        self.assertEqual(result.exit_code, 0)

    def test_powerdownroute(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['-dx', 'powerdownroute', 'blurtpybot'], input="test\n")
        self.assertEqual(result.exit_code, 0)
    ...
    def test_permissions(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['permissions', 'blurtpybot'])
        self.assertEqual(result.exit_code, 0)

    def test_follower(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['follower', 'blurtpybot'])
        self.assertEqual(result.exit_code, 0)

    def test_following(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['following', 'blurtpybot'])
        self.assertEqual(result.exit_code, 0)

    def test_muter(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['muter', 'blurtpybot'])
        self.assertEqual(result.exit_code, 0)
    ...
    def test_muting(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['muting', 'blurtpy'])
        self.assertEqual(result.exit_code, 0)

    def test_allow_disallow(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['-dx', 'allow', '--account', 'blurtpybot', '--permission', 'posting', 'blurtpypy'], input="test\n")
        self.assertEqual(result.exit_code, 0)
    ...
    def test_approvewitness(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['-dx', 'approvewitness', '-a', 'blurtpypy', 'holger80'], input="test\n")
        self.assertEqual(result.exit_code, 0)

    def test_disapprovewitness(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['-dx', 'disapprovewitness',  '-a', 'blurtpypy', 'holger80'], input="test\n")
        self.assertEqual(result.exit_code, 0)

    def test_addproxy(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['-dx', 'setproxy', '-a', 'blurtpypy', 'holger80'], input="test\n")
        self.assertEqual(result.exit_code, 0)
    ...
    def test_newaccount(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['-dx', 'newaccount', 'blurtpy3'], input="test\ntest\n")
        self.assertEqual(result.exit_code, 0)
        result = runner.invoke(cli, ['-dx', 'newaccount', '--owner', 'STM7mLs2hns87f7kbf3o2HBqNoEaXiTeeU89eVF6iUCrMQJFzBsPo',
                                     ...
                                     '--memo', 'STM8Wvi74GYzBKgnUmiLvptzvxmPtXfjGPJL8QY3rebecXaxGGQyV', 'blurtpy3'], input="test\ntest\n")
        self.assertEqual(result.exit_code, 0)                            
    ...
    def test_changekeys(self):
        ...
        result = runner.invoke(cli, ['-dx', 'changekeys', '--owner', 'STM7mLs2hns87f7kbf3o2HBqNoEaXiTeeU89eVF6iUCrMQJFzBsPo',
                                     '--active', 'STM7rUmnpnCp9oZqMQeRKDB7GvXTM9KFvhzbA3AKcabgTBfQZgHZp',
                                     '--posting', 'STM6qGWHsCpmHbphnQbS2yfhvhJXDUVDwnsbnrMZkTqfnkNEZRoLP',
                                     '--memo', 'STM8Wvi74GYzBKgnUmiLvptzvxmPtXfjGPJL8QY3rebecXaxGGQyV', 'blurtpy'], input="test\ntest\n")
        self.assertEqual(result.exit_code, 0)     
    ...
    def test_importaccount(self):
        ...
        result = runner.invoke(cli, ['importaccount', '--roles', '["owner", "active", "posting", "memo"]', 'blurtpy2'], input="test\numybjvCafrt8LdoCjEimQiQ4\n")
        self.assertEqual(result.exit_code, 0)
    ...
    def test_reblog(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['-dto', 'reblog', '@blurtit/firstpost'], input="test\n")
        self.assertEqual(result.exit_code, 0)

    def test_follow_unfollow(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['-dto', 'follow', 'blurtpypy'], input="test\n")
        self.assertEqual(result.exit_code, 0)
        result = runner.invoke(cli, ['-dto', 'unfollow', 'blurtpypy'], input="test\n")
        self.assertEqual(result.exit_code, 0)

    def test_mute_unmute(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['-dto', 'mute', 'blurtpypy'], input="test\n")
        self.assertEqual(result.exit_code, 0)
        result = runner.invoke(cli, ['-dto', 'unfollow', 'blurtpypy'], input="test\n")
        self.assertEqual(result.exit_code, 0)
    ...
    def test_witnesscreate(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['-dx', 'witnesscreate', 'blurtpy', pub_key], input="test\n")
    ...
    def test_claimreward(self):
        ...
        result = runner.invoke(cli, ['-dx', 'claimreward', '--claim_all_blurt'], input="test\n")
    ...
    def test_updatenodes(self):
        runner = CliRunner()
        runner.invoke(cli, ['-o', 'set', 'nodes', self.node_list])
        result = runner.invoke(cli, ['updatenodes', '--blurt', '--test'])
        self.assertEqual(result.exit_code, 0)
        result = runner.invoke(cli, ['updatenodes', '--blurt'])
        self.assertEqual(result.exit_code, 0)
