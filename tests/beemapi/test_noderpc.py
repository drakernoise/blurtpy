...
from blurtapi.exceptions import NumRetriesReached, CallRetriesReached
...
class Testcases(unittest.TestCase):
    ...
    def setUpClass(cls):
        nodelist = NodeList()
        nodelist.update_nodes(blurt_instance=Blurt(node=nodelist.get_nodes(normal=True, appbase=True), num_retries=3))
        cls.nodes = nodelist.get_nodes()
        if "https://api.blurtit.com" in cls.nodes:
            cls.nodes.remove("https://api.blurtit.com")
        cls.nodes_blurtit = ["https://api.blurtit.com"]
        ...
        cls.rpc = NodeRPC(urls=cls.nodes_blurtit)
        # from getpass import getpass
        # self.bts.wallet.unlock(getpass())
        set_shared_blurt_instance(cls.nodes_blurtit)
    ...
    def test_connect_test_node(self):
        rpc = self.rpc
        self.assertIn(rpc.url, self.nodes + self.nodes_blurtit)
        rpc.rpcclose()
        rpc.rpcconnect()
        self.assertIn(rpc.url, self.nodes + self.nodes_blurtit)
    ...
    def test_error_handling(self):
        rpc = NodeRPC(urls=self.nodes_blurtit, num_retries=2, num_retries_call=3)
        with self.assertRaises(
            exceptions.NoMethodWithName
        ):
