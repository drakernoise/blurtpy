...
from blurtapi.noderpc import NodeRPC
...
from .blurtsigner import BlurtSigner
...
from blurtpy.constants import BLURT_VOTE_REGENERATION_SECONDS, BLURT_100_PERCENT, BLURT_1_PERCENT, BLURT_RC_REGEN_TIME, CURVE_CONSTANT, \
...
class BlockChainInstance(object):
    """ Connect to a Graphene network.
        ...
        :param bool use_sc2: When True, a blurtconnect object is created. Can be used for
            broadcast posting op or creating hot_links (default is False)
        :param BlurtConnect blurtconnect: A BlurtConnect object can be set manually, set use_sc2 to True
        :param dict custom_chains: custom chain which should be added to the known chains

        Three wallet operation modes are possible:

        * **Wallet Database**: Here, the blurtlibs load the keys from the
        ...
        * **Force keys**: This more is for advanced users and
        ...
        http://geo.blurt.pl. Default settings can be changed with:
    ...
    def __init__(self,
        ...
        """Init blurt
            ...
            :param int num_retries_call: Repeat num_retries_call times a rpc call on node error (default is 5)
                :param int timeout: Timeout setting for https nodes (default is 60)
            :param bool use_sc2: When True, a blurtconnect object is created. Can be used for broadcast
        ...
        self.blurtconnect = kwargs.get("blurtconnect", None)
        self.use_sc2 = bool(kwargs.get("use_sc2", False))
        self.blurtsigner = kwargs.get("blurtsigner", None)
        ...
        # set blurtconnect
        if self.blurtconnect is not None and not isinstance(self.blurtconnect, (BlurtSigner)):
            raise ValueError("blurtconnect musst be BlurtConnect object")
        if self.blurtsigner is not None and not isinstance(self.blurtsigner, (BlurtSigner)):
            raise ValueError("blurtsigner musst be BlurtSigner object")        
        if self.blurtconnect is not None and not self.use_sc2:
            self.use_sc2 = True
        elif self.blurtsigner is None and self.use_hs:
            self.blurtsigner = BlurtSigner(blockchain_instance=self, **kwargs)
        elif self.blurtsigner is not None and not self.use_hs:
            self.use_hs = True            
    ...
    def connect(self,
                ...
                rpcpassword="",
                **kwargs):
        """ Connect to Blurt network (internal use only)
        """
        if not node:
            node = self.get_default_nodes()
            if not bool(node):
                raise ValueError("A Blurt node needs to be provided!")
    ...
    def refresh_data(self, chain_property, force_refresh=False, data_refresh_time_seconds=None):
        """ Read and stores blurt blockchain parameters
        ...
        if chain_property == "dynamic_global_properties":
            if not self.offline:
                if self.data['last_refresh_dynamic_global_properties'] is not None and not force_refresh and self.data["last_node"] == self.rpc.url:
                    if (datetime.now(timezone.utc) - self.data['last_refresh_dynamic_global_properties']).total_seconds() < self.data_refresh_time_seconds:
                        return            
            ...
            if not self.offline:
                if self.data['last_refresh_feed_history'] is not None and not force_refresh and self.data["last_node"] == self.rpc.url:
                    if (datetime.now(timezone.utc) - self.data['last_refresh_feed_history']).total_seconds() < self.data_refresh_time_seconds:
                        return
            ...
            if not self.offline:
                if self.data['last_refresh_hardfork_properties'] is not None and not force_refresh and self.data["last_node"] == self.rpc.url:
                    if (datetime.now(timezone.utc) - self.data['last_refresh_hardfork_properties']).total_seconds() < self.data_refresh_time_seconds:
                        return
            ...
            if not self.offline:
                if self.data['last_refresh_witness_schedule'] is not None and not force_refresh and self.data["last_node"] == self.rpc.url:
                    if (datetime.now(timezone.utc) - self.data['last_refresh_witness_schedule']).total_seconds() < 3:
                        return
            ...
            if not self.offline:
                if self.data['last_refresh_config'] is not None and not force_refresh and self.data["last_node"] == self.rpc.url:
                    if (datetime.now(timezone.utc) - self.data['last_refresh_config']).total_seconds() < self.data_refresh_time_seconds:
                        return
            ...
            if not self.offline:
                if self.data['last_refresh_reward_funds'] is not None and not force_refresh and self.data["last_node"] == self.rpc.url:
                    if (datetime.now(timezone.utc) - self.data['last_refresh_reward_funds']).total_seconds() < self.data_refresh_time_seconds:
                        return
    ...
    def get_reward_funds(self, use_stored_data=True):
        ...
        if self.is_blurt:
            return self.rpc.get_reward_fund('post', api='database')
        
        # Debugging why is_blurt might be False
        # print(f"DEBUG: is_blurt={self.is_blurt}, config_keys={list(self.get_config().keys()) if self.get_config() else 'None'}")
    ...
    def get_rc_cost(self, resource_count):
        ...
        rc_regen = int(Amount(dyn_param["total_vesting_shares"], blockchain_instance=self)) / \
            (BLURT_RC_REGEN_TIME / self.get_block_interval())
        total_cost = 0
        if rc_regen == 0:
            return total_cost
    ...
    def _max_vote_denom(self, use_stored_data=True):
        ...
        max_vote_denom = vote_power_reserve_rate * BLURT_VOTE_REGENERATION_SECONDS
        return max_vote_denom

    def _calc_resulting_vote(self, voting_power=BLURT_100_PERCENT, vote_pct=BLURT_100_PERCENT, use_stored_data=True):
        # determine voting power used
        used_power = int((voting_power * abs(vote_pct)) / BLURT_100_PERCENT * (60 * 60 * 24))
    ...
    def vests_to_rshares(self, vests, voting_power=BLURT_100_PERCENT, vote_pct=BLURT_100_PERCENT, subtract_dust_threshold=True, use_stored_data=True):
        ...
        rshares = int(math.copysign(vests * 1e6 * used_power / BLURT_100_PERCENT, vote_pct))
    ...
    def token_power_to_token_backed_dollar(self, token_power, post_rshares=0, voting_power=BLURT_100_PERCENT, vote_pct=BLURT_100_PERCENT, not_broadcasted_vote=True, use_stored_data=True):
        """ Obtain the resulting Token backed dollar vote value from Token power
            ...
            :param number blurt_power: Token Power
    ...
    def is_blurt(self):
        ...
        return 'BLURT_CHAIN_ID' in config or 'BLURT_BLOCKCHAIN_VERSION' in config
    ...
    def switch_blockchain(self, blockchain, update_nodes=False):
        """ Switches the connected blockchain. Can be either blurt or blurt.

            :param str blockchain: can be "blurt" or "blurt"
            :param bool update_nodes: When true, the nodes are updated, using
                NodeList.update_nodes()
        """
        assert blockchain in ["blurt", "blurt"]
        ...
        if update_nodes:
            nodelist.update_nodes()
        if blockchain == "blurt":
            self.set_default_nodes(nodelist.get_blurt_nodes())
        else:
            self.set_default_nodes(nodelist.get_blurt_nodes())
    ...
    def set_password_storage(self, password_storage):
        """ Set the password storage mode.
            ...
            python -m keyring set blurtpy wallet password
    ...
    def finalizeOp(self, ops, account, permission, **kwargs):
        """ This method obtains the required private keys if present in
            ...
            .. note:: This uses :func:`BlockChainInstance.txbuffer` as instance of
                :class:`blurtpy.transactionbuilder.TransactionBuilder`.
    ...
    def broadcast(self, tx=None, trx_id=True):
        """ Broadcast a transaction to the Blurt/Blurt network
    ...
    def newWallet(self, pwd):
        """ Create a new wallet. This method is basically only calls
            :func:`blurtpy.wallet.Wallet.create`.
    ...
    def claim_account(self, creator, fee=None, **kwargs):
        ...
        if not creator:
            raise ValueError(
                ...
                "creator=x, or set the default_account using blurtpypy")
    ...
    def create_claimed_account(
        ...
        **kwargs
        ...
        """ Create new claimed account on Blurt

            The brainkey/password can be used to recover all generated keys
            (see :class:`blurtgraphenebase.account` for more details.
    ...
    def create_account(
        ...
        **kwargs
        ...
        """ Create new account on Blurt/Blurt
        ...
        if self.hardfork >= 20:
            required_fee_blurt = Amount(props["account_creation_fee"], blockchain_instance=self)
        else:
            required_fee_blurt = Amount(props["account_creation_fee"], blockchain_instance=self) * 30    
        op = {
            "fee": required_fee_blurt,
            "creator": creator["name"],
            "new_account_name": account_name,
            'owner': {'account_auths': owner_accounts_authority,
                      'key_auths': owner_key_authority,
    ...
    def custom_json(self,
        ...
        """ Create a custom json operation
            ...
            .. code-block:: python
               ...
               blurt.custom_json("id", "json_data",
    ...
    def post(self,
        ...
        :param str app: (Optional) Name of the app which are used for posting
            when not set, blurtpy/<version> is used
        ...
        if app:
            ...
            json_metadata.update({'app': 'blurtpy/%s' % (blurtpy_version)})
        ...
        if parse_body:
            ...
            for url in get_urls(body):
                ...
                if os.path.splitext(url)[1].lower() in img_exts:
                    image.append(url)
                elif url[:25] == "https://images.blurt.blog/":
                    image.append(url)
                else:
                    links.append(url)
    ...
    def vote(self, weight, identifier, account=None, **kwargs):
        ...
        vote_weight = int(float(weight) * BLURT_1_PERCENT)
        if vote_weight > BLURT_100_PERCENT:
            vote_weight = BLURT_100_PERCENT
        if vote_weight < -BLURT_100_PERCENT:
            vote_weight = -BLURT_100_PERCENT
    ...
    def _build_comment_options_op(self, author, permlink, options,
        ...
        if beneficiaries:
            ...
            for b in beneficiaries:
                ...
                if 'weight' not in b:
                    b['weight'] = BLURT_100_PERCENT
                if len(b['account']) > 16:
                    raise ValueError(
                        "beneficiaries error, account name length >16!"
                    )
                if b['weight'] < 1 or b['weight'] > BLURT_100_PERCENT:
                    raise ValueError(
            ...
            if weight_sum > BLURT_100_PERCENT:
                raise ValueError(
        ...
        if self.is_blurt:
            ...
            comment_op = operations.Comment_options(
                **{
                    ...
                    int(options.get("percent_blurt_dollars", BLURT_100_PERCENT)),
                    "allow_votes":
                    options.get("allow_votes", True),
                    "allow_curation_rewards":
                    options.get("allow_curation_rewards", True),
