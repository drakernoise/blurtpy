...
from blurtgraphenebase.py23 import PY2, PY3
from .objects import (
    Operation,
    Memo,
    Amount,
            ...
            super(Comment_options, self).__init__(
                OrderedDict([
                    ...
                    ('percent_blurt_dollars',
                     Uint16(int(kwargs["percent_blurt_dollars"]))),
                    ('allow_votes', Bool(bool(kwargs["allow_votes"]))),
                    ('allow_curation_rewards',
                     Bool(bool(kwargs["allow_curation_rewards"]))),
                    ('extensions', extensions),
...
class Claim_reward_balance(GrapheneObject):
    def __init__(self, *args, **kwargs):
        ...
        prefix = kwargs.get("prefix", default_prefix)
        json_str = kwargs.get("json_str", False)
        if "reward_sbd" in kwargs and "reward_blurt" in kwargs:
            super(Claim_reward_balance, self).__init__(
                OrderedDict([
                    ('account', String(kwargs["account"])),
                    ('reward_blurt', Amount(kwargs["reward_blurt"], prefix=prefix, json_str=json_str)),
                    ('reward_sbd', Amount(kwargs["reward_sbd"], prefix=prefix, json_str=json_str)),
                    ('reward_vests', Amount(kwargs["reward_vests"], prefix=prefix)),
                ]))
        elif "reward_hbd" in kwargs and "reward_blurt" in kwargs:
            super(Claim_reward_balance, self).__init__(
                OrderedDict([
                    ('account', String(kwargs["account"])),
...
class Escrow_transfer(GrapheneObject):
    def __init__(self, *args, **kwargs):
        ...
        if "hbd_amount" in kwargs and "blurt_amount" in kwargs:
            super(Escrow_transfer, self).__init__(
                OrderedDict([
                    ...
                    ('blurt_amount', Amount(kwargs["blurt_amount"], prefix=prefix, json_str=json_str)),
                    ('fee', Amount(kwargs["fee"], prefix=prefix, json_str=json_str)),
            ...
            super(Escrow_transfer, self).__init__(
                OrderedDict([
                    ...
                    ('blurt_amount', Amount(kwargs["blurt_amount"], prefix=prefix)),
...
class Escrow_release(GrapheneObject):
    def __init__(self, *args, **kwargs):
        ...
        prefix = kwargs.get("prefix", default_prefix)
        json_str = kwargs.get("json_str", False)
        if "blurt_amount" in kwargs and "hbd_amount" in kwargs:
