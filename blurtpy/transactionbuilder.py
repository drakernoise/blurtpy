...
from blurtbase.ledgertransactions import Ledger_Transaction
...
class TransactionBuilder(dict):
    """ This class simplifies the creation of transactions by adding
        ...
        :param Blurt/Blurt blockchain_instance: If not set, shared_blockchain_instance() is used
    ...
    def appendSigner(self, account, permission):
        ...
        if self.blockchain.wallet.locked():
            raise WalletLocked()
        if self.blockchain.use_sc2 and self.blockchain.blurtconnect is not None:
            self.blockchain.blurtconnect.set_username(account["name"], permission)
            return
    ...
    def broadcast(self, max_block_age=-1, trx_id=True):
        """ Broadcast a transaction to the blurt network
        ...
        try:
            self.blockchain.rpc.set_next_node_on_empty_reply(False)
            if self.blockchain.use_sc2:
                ret = self.blockchain.blurtconnect.broadcast(self["operations"])
