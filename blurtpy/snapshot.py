...
from blurtpy.utils import formatTimeString, formatTimedelta, remove_from_dict, reputation_to_score, addTzInfo, parse_time
...
from blurtpy.constants import BLURT_VOTE_REGENERATION_SECONDS, BLURT_1_PERCENT, BLURT_100_PERCENT
...
class AccountSnapshot(list):
    ...
    def reset(self):
        ...
        import blurtbase.operationids
        self.ops_statistics = blurtbase.operationids.operations.copy()
    ...
    def parse_op(self, op, only_ops=[], enable_rewards=False, enable_out_votes=False, enable_in_votes=False):
        ...
        if op['type'] == "account_create":
            fee_blurt = Amount(op['fee'], blockchain_instance=self.blockchain).amount
            ...
            if op['creator'] == self.account["name"]:
                self.update(ts, 0, 0, 0, fee_blurt * (-1), 0)
                return
            ...
            if op['creator'] == self.account["name"]:
                delegation = {'account': op['new_account_name'], 'amount':
                              Amount(op['delegation'], blockchain_instance=self.blockchain)}
                self.update(ts, 0, 0, delegation, fee_blurt * (-1), 0)
                return
            ...
            blurt = Amount(op['amount'], blockchain_instance=self.blockchain)
            from blurtpy import Blurt
            if isinstance(self.blockchain, Blurt):
                vests = self.blockchain.sp_to_vests(blurt.amount, timestamp=ts)
            else:
                vests = self.blockchain.hp_to_vests(blurt.amount, timestamp=ts)
            if op['from'] == self.account["name"] and op['to'] == self.account["name"]:
                self.update(ts, vests, 0, 0, blurt * (-1), 0)  # power up from and to given account
                ...
                self.update(ts, 0, 0, 0, blurt * (-1), 0)  # power up to another account
            return
    ...
    def build_vp_arrays(self):
        ...
        self.downvote_vp = [BLURT_100_PERCENT]
        ...
        for (ts, weight) in zip(self.out_vote_timestamp, self.out_vote_weight):
            ...
            if ts > HF_21 and weight < 0:
                ...
                if self.downvote_vp[-1] < BLURT_100_PERCENT:
                    regenerated_vp = ((ts - self.downvote_vp_timestamp[-1]).total_seconds()) * BLURT_100_PERCENT / BLURT_VOTE_REGENERATION_SECONDS
                    self.downvote_vp[-1] += int(regenerated_vp)

                if self.downvote_vp[-1] > BLURT_100_PERCENT:
                    self.downvote_vp[-1] = BLURT_100_PERCENT
                    ...
                    self.downvote_vp.append(BLURT_100_PERCENT)

                # Add charged downvote VP just before new Vote
                self.downvote_vp_timestamp.append(ts-timedelta(seconds=1))
                self.downvote_vp.append(min([BLURT_100_PERCENT, self.downvote_vp[-1] + regenerated_vp]))

                self.downvote_vp[-1] -= self.blockchain._calc_resulting_vote(BLURT_100_PERCENT, weight) * 4
                ...
                if self.downvote_vp[-1] < 0:
                    ...
                    if self.vp[-1] < BLURT_100_PERCENT:
                        regenerated_vp = ((ts - self.vp_timestamp[
                            -1]).total_seconds()) * BLURT_100_PERCENT / BLURT_VOTE_REGENERATION_SECONDS
                        self.vp[-1] += int(regenerated_vp)

                    if self.vp[-1] > BLURT_100_PERCENT:
                        self.vp[-1] = BLURT_100_PERCENT
                        ...
                        self.vp.append(BLURT_100_PERCENT)
                    if self.vp[-1] == BLURT_100_PERCENT and ts - self.vp_timestamp[-1] > timedelta(seconds=1):
                        # Add charged VP just before new Vote
                        self.vp_timestamp.append(ts-timedelta(seconds=1))
                        self.vp.append(min([BLURT_100_PERCENT, self.vp[-1] + regenerated_vp]))
                ...
                if self.vp[-1] < BLURT_100_PERCENT:
                    regenerated_vp = ((ts - self.vp_timestamp[-1]).total_seconds()) * BLURT_100_PERCENT / BLURT_VOTE_REGENERATION_SECONDS
                    self.vp[-1] += int(regenerated_vp)
