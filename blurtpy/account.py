...
from blurtgraphenebase.account import PrivateKey, PublicKey, PasswordKey
from blurtgraphenebase.py23 import bytes_types, integer_types, string_types, text_type
from blurtpy.constants import BLURT_VOTE_REGENERATION_SECONDS, BLURT_1_PERCENT, BLURT_100_PERCENT, BLURT_VOTING_MANA_REGENERATION_SECONDS
...
class Account(BlockchainObject):
    """ This class allows to easily access Account data
        ...
        :param Blurt/Blurt blockchain_instance: Blurt or Blurt
               instance
        ...
        :rtype: dictionary
        :raises blurtpy.exceptions.AccountDoesNotExistsException: if account
                does not exist
        ...
        .. code-block:: python
            ...
            >>> stm = Blurt(node=nodelist.get_blurt_nodes())
    ...
    def __init__(
        ...
        **kwargs
        ...
        :param str account: Name of the account
        :param Blurt blockchain_instance: Blurt
               instance
        :param bool lazy: Use lazy loading
        :param bool full: Obtain all account data including orders, positions,
               etc.
    ...
    def _parse_json_data(self, account):
        ...
        parse_times = [
            ...
            "last_account_recovery", "last_vote_time", "blurt_seconds_last_update", "blurt_last_interest_payment",
            "savings_blurt_seconds_last_update", "savings_blurt_last_interest_payment", "next_vesting_withdrawal",
            "last_market_bandwidth_update", "last_post", "last_root_post", "last_bandwidth_update",
            "blurt_seconds_last_update", "blurt_last_interest_payment", "savings_blurt_seconds_last_update",
            "savings_blurt_last_interest_payment"            
        ]
    ...
    def get_similar_account_names(self, limit=5):
        """ Returns ``limit`` account names similar to the current account
            ...
            This is a wrapper around :func:`blurtpy.blockchain.Blockchain.get_similar_account_names()`
    ...
    @property
    def tp(self):
        """ Returns the accounts Blurt/Blurt Power
        """
        return self.get_token_power()
    ...
    def get_reputation(self):
        """ Returns the account reputation in the (blurtit) normalized form
        """
        if not self.blockchain.is_connected():
            return None
    ...
    def get_manabar(self):
        ...
        last_update = datetime.fromtimestamp(last_update_time, timezone.utc)
        diff_in_seconds = (addTzInfo(datetime.now(timezone.utc)) - addTzInfo(last_update)).total_seconds()
        current_mana = int(last_mana + diff_in_seconds * max_mana / BLURT_VOTING_MANA_REGENERATION_SECONDS)
    ...
    def get_voting_power(self, with_regeneration=True):
        ...
        if "voting_manabar" in self:
            ...
            if with_regeneration:
                ...
                diff_in_seconds = (addTzInfo(datetime.now(timezone.utc)) - (last_vote_time)).total_seconds()
                regenerated_vp = diff_in_seconds * BLURT_100_PERCENT / BLURT_VOTE_REGENERATION_SECONDS / 100
            else:
                regenerated_vp = 0
    ...
    def get_token_power(self, only_own_vests=False, use_stored_data=True):
        """ Returns the account Blurt/Blurt power (amount of staked token + delegations)
    ...
    def get_vote_pct_for_BLURT(self, blurt, post_rshares=0, voting_power=None, blurt_power=None, not_broadcasted_vote=True):
        """ Returns the voting percentage needed to have a vote worth a given number of BLURT.
            ...
            the given BLURT value is too high for that account

            :param blurt: The amount of BLURT in vote value
            :type blurt: str, int, amount.Amount
    ...
    def get_vote_pct_for_vote_value(self, token_units, post_rshares=0, voting_power=None, token_power=None, not_broadcasted_vote=True):
        """ Returns the voting percentage needed to have a vote worth a given number of Blurt/Blurt token units
            ...
            :param token_units: The amount of blurt/BLURT in vote value
    ...
    def get_recharge_timedelta(self, voting_power_goal=100, starting_voting_power=None):
        ...
        if missing_vp < 0:
            return 0
        recharge_seconds = missing_vp * 100 * BLURT_VOTING_MANA_REGENERATION_SECONDS / BLURT_100_PERCENT
    ...
    def get_recharge_time(self, voting_power_goal=100, starting_voting_power=None):
        ...
        """
        return addTzInfo(datetime.now(timezone.utc)) + self.get_recharge_timedelta(voting_power_goal, starting_voting_power)
    ...
    def get_manabar_recharge_timedelta(self, manabar, recharge_pct_goal=100):
        ...
        if missing_rc_pct < 0:
            return 0
        recharge_seconds = missing_rc_pct * 100 * BLURT_VOTING_MANA_REGENERATION_SECONDS / BLURT_100_PERCENT
        return timedelta(seconds=recharge_seconds)
    ...
    def get_manabar_recharge_time(self, manabar, recharge_pct_goal=100):
        ...
        """
        return addTzInfo(datetime.now(timezone.utc)) + self.get_manabar_recharge_timedelta(manabar, recharge_pct_goal)
    ...
    def get_feed(self, start_entry_id=0, limit=100, raw_data=False, short_entries=False, account=None):
        """ Returns a list of items in an account’s feed
            ...
            .. code-block:: python
                ...
                >>> nodelist.update_nodes()
                >>> stm = Blurt(node=nodelist.get_blurt_nodes())
                >>> account = Account("blurtit", blockchain_instance=stm)
        ...
        if not self.blockchain.is_connected():
            return None
        from blurtpy.discussions import Discussions, Query
    ...
    def get_account_posts(self, sort="feed", limit=20, account=None, observer=None, raw_data=False):
        ...
        if not self.blockchain.is_connected():
            raise OfflineHasNoRPCException("No RPC available in offline mode!")
        from blurtpy.comment import AccountPosts
    ...
    def available_balances(self):
        """ List balances of an account. This call returns instances of
            :class:`blurtpy.amount.Amount`.
        """
        if "blurt_balance" in self:
            amount_list = ["balance", "blurt_balance", "vesting_shares"]
        elif "blurt_balance" in self:
            amount_list = ["balance", "blurt_balance", "vesting_shares"]
        else:
    ...
    def saving_balances(self):
        savings_amount = []
        if "savings_blurt_balance" in self:
            amount_list = ["savings_balance", "savings_blurt_balance"]
        elif "savings_blurt_balance" in self:
            amount_list = ["savings_balance", "savings_blurt_balance"]
        else:
    ...
    def reward_balances(self):
        if "reward_blurt_balance" in self and "reward_blurt_balance" in self:
            amount_list = ["reward_blurt_balance", "reward_blurt_balance", "reward_vesting_balance"]
        elif "reward_blurt_balance" in self and "reward_blurt_balance" in self:
            amount_list = ["reward_blurt_balance", "reward_blurt_balance", "reward_vesting_balance"]
    ...
    def get_balance(self, balances, symbol):
        """ Obtain the balance of a specific Asset. This call returns instances of
            :class:`blurtpy.amount.Amount`. Available balance types:
            ...
            * "total"
            ...
            .. code-block:: python
                ...
                >>> nodelist.update_nodes()
                >>> stm = Blurt(node=nodelist.get_blurt_nodes())
                >>> account = Account("blurtpy.app", blockchain_instance=stm)
                >>> account.get_balance("rewards", "blurt")
                0.000 blurt
    ...
    def interest(self):
        ...
        if "blurt_last_interest_payment" in self:
            last_payment = (self["blurt_last_interest_payment"])
            next_payment = last_payment + timedelta(days=30)
            interest_rate = self.blockchain.get_dynamic_global_properties()[
                "blurt_interest_rate"] / 100  # percent
            interest_amount = (interest_rate / 100) * int(
                int(self["blurt_seconds"]) / (60 * 60 * 24 * 356)) * 10**-3
        elif "blurt_last_interest_payment" in self:
            last_payment = (self["blurt_last_interest_payment"])
            next_payment = last_payment + timedelta(days=30)
            interest_rate = self.blockchain.get_dynamic_global_properties()[
                "blurt_interest_rate"] / 100  # percent
    ...
    def get_conversion_requests(self, account=None):
        """ Returns a list of BLURT conversion request
        ...
        if not self.blockchain.is_connected():
            raise OfflineHasNoRPCException("No RPC available in offline mode!")
        self.blockchain.rpc.set_next_node_on_empty_reply(False)
        if self.blockchain.rpc.get_use_appbase() and "blurt_balance" in self:
            return self.blockchain.rpc.find_blurt_conversion_requests({'account': account}, api="database")['requests']
        elif self.blockchain.rpc.get_use_appbase() and "blurt_balance" in self:
            return self.blockchain.rpc.find_blurt_conversion_requests({'account': account}, api="database")['requests']
        else:
        ...
        if until is not None:
            ...
            try:
                ...
                if isinstance(op_count, list) and len(op_count) > 0 and len(op_count[0]) > 0:
                    if self.blockchain.rpc.url == "https://api.blurt.blog":
    ...
    def _get_operation_filter(self, only_ops=[], exclude_ops=[]):
        from blurtbase.operationids import operations
    ...
    def history(
        ...
        if first > max_index:
            ...
            first = start_index + _limit - 1
        elif first < _limit and self.blockchain.rpc.url == "https://api.blurt.blog":
            first = _limit - 1
        elif first < _limit and self.blockchain.rpc.url != "https://api.blurt.blog":
            first = _limit
        last_round = False
        ...
        if self.blockchain.rpc.url == 'https://api.blurt.blog' and (len(only_ops) > 0 or len(exclude_ops) > 0):
        ...
        while True:
            # RPC call
            if first < _limit - 1 and self.blockchain.rpc.url == "https://api.blurt.blog":
                first = _limit - 1
            elif first < _limit and self.blockchain.rpc.url != "https://api.blurt.blog":
                first = _limit
            batch_count = 0
    ...
    def history_reverse(
        ...
        while True:
            # RPC call
            if first - _limit < 0 and self.blockchain.rpc.url == 'https://api.blurt.blog':
                _limit = first + 1
            elif first - _limit < 0 and self.blockchain.rpc.url != 'https://api.blurt.blog':
                _limit = first
            batch_count = 0
            for item in self.get_account_history(first, _limit, start=None, stop=None, order=-1, only_ops=only_ops, exclude_ops=exclude_ops, raw_output=raw_output):
                batch_count += 1
    ...
    def follow(self, other, what=["blog"], account=None):
        """ Follow/Unfollow/Mute/Unmute another account's blog
            ...
            :param str/list other: Follow this account / accounts (only blurt)
        ...
        """ Update an account's profile in json_metadata
            ...
            .. code-block:: js

                {
                    ...
                    'website': 'https://github.com/holgern/blurtpy'
                }
    ...
    def convert(self, amount, account=None, request_id=None):
        """ Convert BlurtDollars to Blurt (takes 3.5 days to settle)

            :param float amount: amount of BLURT to convert
            :param str account: (optional) the source account for the transfer
                if not ``default_account``
    ...
    def set_withdraw_vesting_route(self,
        ...
        op = operations.Set_withdraw_vesting_route(
            **{
                ...
                "to_account": to,
                "percent": int(percentage * BLURT_1_PERCENT),
                "auto_vest": auto_vest
            })
    ...
    def blog_history(self, limit=None, start=-1, reblogs=True, account=None):
        """ Stream the blog entries done by an account in reverse time order.
            ...
            :param bool reblogs: (optional) if set `True` (default)
                reblogs / reblurts are included. If set `False`,
                reblogs/reblurts are omitted.
            ...
            .. code-block:: python
                ...
                nodelist.update_nodes()
                stm = Blurt(node=nodelist.get_blurt_nodes())
                acc = Account("blurtitblog", blockchain_instance=stm)
...
class Accounts(AccountsObject):
    """ Obtain a list of accounts
        ...
        :param Blurt/Blurt blockchain_instance: Blurt() or Blurt() instance to use when
