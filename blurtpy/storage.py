...
def generate_config_store(config, blockchain="blurt"):
    #: Default configuration
    nodelist = NodeList()
    if blockchain == "blurt":
        nodes = nodelist.get_blurt_nodes(testnet=False)
    ...
    config.setdefault("sc2_api_url", "https://api.blurtconnect.com/api/")
    config.setdefault("oauth_base_url", "https://api.blurtconnect.com/oauth2/")
    config.setdefault("hs_api_url", "https://blurtsigner.com/api/")
    config.setdefault("hs_oauth_base_url", "https://blurtsigner.com/oauth2/")
    config.setdefault("default_canonical_url", "https://blurt.blog")
...
def get_default_config_store(*args, **kwargs):
    return generate_config_store(SqliteConfigurationStore, blockchain="blurt")(*args, **kwargs)
