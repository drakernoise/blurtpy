...
class Market(dict):
    """ This class allows to easily access Markets on the blockchain for trading, etc.
        ...
        * ``base`` and ``quote`` are valid assets (according to :class:`blurtpy.asset.Asset`)
        ...
        * ``base-quote`` separated with ``-``
        ...
        .. note:: Throughout this library, the ``quote`` symbol will be
                  ...
                  :func:`blurtpy.market.Market.buy`, you will sell/buy **only
    ...
    def __init__(
        ...
        **kwargs
        ...
        Init Market
            ...
            :param blurtpy.asset.Asset base: Base asset
            :param blurtpy.asset.Asset quote: Quote asset
        """
    ...
    def ticker(self, raw_data=False):
        ...
        if "blurt_volume" in ticker:
            data["blurt_volume"] = Amount(ticker["blurt_volume"], blockchain_instance=self.blockchain)
        elif "blurt_volume" in ticker:
            data["blurt_volume"] = Amount(ticker["blurt_volume"], blockchain_instance=self.blockchain)
    ...
    def volume24h(self, raw_data=False):
        ...
        if raw_data:
            return volume
        if "sbd_volume" in volume and "blurt_volume" in volume:
            return {
                self["base"]["symbol"]: Amount(volume["sbd_volume"], blockchain_instance=self.blockchain),
                self["quote"]["symbol"]: Amount(volume["blurt_volume"], blockchain_instance=self.blockchain)
            }
        elif "hbd_volume" in volume and "blurt_volume" in volume:
            return {
    ...
    def orderbook(self, limit=25, raw_data=False):
        """ Returns the order book for TBD/BLURT market.
            ...
            .. note:: Each bid is an instance of
                class:`blurtpy.price.Order` and thus carries the keys
