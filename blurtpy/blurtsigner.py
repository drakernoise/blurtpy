...
from blurtstorage.exceptions import KeyAlreadyInStoreException, WalletLocked

log = logging.getLogger(__name__)


class BlurtSigner(object):
    """ BlurtSigner
        ...
        .. code-block:: python
            ...
            hs = BlurtSigner(client_id="blurtpy.app")
            blurt = Blurt(BlurtSigner=hs)
            blurt.wallet.unlock("supersecret-passphrase")
            post = Comment("author/permlink", blockchain_instance=blurt)
            post.upvote(voter="test")  # replace "test" with your account

        Examples for creating BlurtSigner urls for broadcasting in browser:
        ...
        .. testoutput::
            ...
            from blurtpy.BlurtSigner import BlurtSigner
            from pprint import pprint
            stm = Blurt(nobroadcast=True, unsigned=True)
            hs = BlurtSigner(blockchain_instance=stm)
    ...
    def __init__(self, blockchain_instance=None, *args, **kwargs):
        ...
        if "token" in kwargs and len(kwargs["token"]) > 0:
            from blurtstorage import InRamPlainTokenStore
            ...
            """ If no keys are provided manually we load the SQLite
                keyStorage
            """
            from blurtstorage import SqliteEncryptedTokenStore
    ...
    def setToken(self, loadtoken):
        """ This method is strictly only for in memory token that are
            passed to Wallet/Blurt with the ``token`` argument
        """
        log.debug(
    ...
    def get_login_url(self, redirect_uri, **kwargs):
        """ Returns a login url for receiving token from BlurtSigner
        """
    ...
    def me(self, username=None):
        """ Calls the me function from BlurtSigner

        .. code-block:: python

            from blurtpy.BlurtSigner import BlurtSigner
            hs = BlurtSigner()
            hs.blurt.wallet.unlock("supersecret-passphrase")
            hs.me(username="test")
                ...
                [
                    [
                        'vote', {
                                    'voter': 'gandalf',
                                    'author': 'gtg',
                                    'permlink': 'blurt-pressure-4-need-for-speed',
                                    'weight': 10000
                                }
                    ]
                ]
