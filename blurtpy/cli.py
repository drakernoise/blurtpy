...
from blurtpy.version import version as __version__
from blurtpy.asciichart import AsciiChart
...
@shell(prompt='blurtpypy> ', intro='Starting blurtpypy... (use help to list all commands)', chain=True)
# @click.group(chain=True)
@click.option(
    '--node', '-n', default="", help="URL for public Blurt API (e.g. https://api.blurt.blog)")
@click.option(
...
@click.option(
    '--create-link', '-l', is_flag=True, default=False, help="Creates blurtsigner links from all broadcast operations")
@click.option(
    '--blurt', '-s', is_flag=True, default=False, help="Connect to the Blurt blockchain")
@click.option(
    '--blurt', '-h', is_flag=True, default=False, help="Connect to the Blurt blockchain")
@click.option(
...
@click.option(
    '--token', '-t', is_flag=True, default=False, help="Uses a blurtsigner token to broadcast (only broadcast operation with posting permission)")
@click.option(
...
def cli(node, offline, no_broadcast, no_wallet, unsigned, create_link, blurt, keys, use_ledger, path, token, expires, verbose):
    ...
    debug = verbose > 0
    blurt = False
    if not blurt and not blurt:
        config = get_default_config_store()
        if config["default_chain"].lower() == "blurt":
            blurt = True
...
def set(key, value):
    ...
    if key == "default_account":
        ...
        if is_keyring_available() and value == "keyring":
            ...
            import keyring
            try:
                keyring.delete_password("blurtpy", "wallet")
...
def about():
    """ About blurtpypy"""
    print("")
    print("blurtpypy version: %s" % __version__)
    print("")
    print("By @holger80")
    print("")
...
def currentnode(version, url):
    ...
    if not offline:
        ...
        t.add_row(["Version", "blurtpy is in offline mode..."])
    print(t)
...
@click.option('--network', '-n', help='Network index, when using BIP39, 0 for blurt and 13 for blurt, (default is 13)', default=13)
...
def addtoken(name, unsafe_import_token):
    ...
    if stm.rpc is not None:
        stm.rpc.rpcconnect()
    sc2 = BlurtSigner(blockchain_instance=stm)
...
def upvote(post, account, weight, export):
    ...
    except exceptions.VotingInvalidOnArcblurtdPost:
...
def powerdown(amount, account, export):
    """Power down (start withdrawing VESTS from Blurt POWER)
...
def delegate(amount, to_account, account, export):
    ...
    try:
        ...
        if amount.symbol == stm.token_symbol and isinstance(stm, Blurt):
            amount = stm.sp_to_vests(float(amount))
        elif amount.symbol == stm.token_symbol and isinstance(stm, Blurt):
...
def followlist(follow_type, account, limit):
    ...
    On Blurt, follow type can also be one the following: blacklisted, follow_blacklist, muted, or follow_muted
    """
    stm = shared_blockchain_instance()
    if stm.rpc is not None:
        stm.rpc.rpcconnect()
...
def message(message_file, account, verify):
    ...
    if message_file is not None:
        ...
        print("Please store the signed message into a text file and append the file path to blurtpypy message -v")
        return
    else:
...
def post(markdown_file, account, title, permlink, tags, reply_identifier, community, canonical_url, beneficiaries, percent_blurt_dollars, percent_hbd, max_accepted_payout, no_parse_body, no_patch_on_edit, export):
    ...
    title: your title
    tags: tag1,tag2
    community: blurt-100000
    beneficiaries: blurtpypy:5%,holger80:5%
    ---
    ...
    if "canonical_url" in parameter:
        ...
        json_metadata["canonical_url"] = stm.config["default_canonical_url"] or "https://blurt.blog"
    ...
    if comment is None or no_patch_on_edit:
        ...
        tx = stm.post(title, patch_text, author=author, permlink=permlink, reply_identifier=reply_identifier, community=community,
                      tags=tags, json_metadata=json_metadata, parse_body=False, app='blurtpypy/%s' % (__version__))
    if stm.unsigned and stm.nobroadcast and stm.blurtconnect is not None:
        tx = stm.blurtconnect.url_from_tx(tx)
    elif stm.unsigned and stm.nobroadcast and stm.blurtsigner is not None:
        tx = stm.blurtsigner.url_from_tx(tx)
...
def witnesscreate(witness, pub_signing_key, maximum_block_size, account_creation_fee, sbd_interest_rate, hbd_interest_rate, url, export):
    ...
    if not unlock_wallet(stm):
        return
    if stm.is_blurt and stm.hardfork >= 24:
...
def customjson(jsonid, json_data, account, active, export):
    """Broadcasts a custom json
        ...
        e.g. blurtpypy customjson -a holger80 dw-heist username holger80 amount 100
    """
    if jsonid is None:
...
def info(objects):
    ...
    for obj in objects:
        ...
        # Post identifier
            ...
            b = Blockchain(blockchain_instance=stm)
            from blurtapi.exceptions import UnknownTransaction
            try:
...
def draw(block, trx_id, draws, participants, hashtype, separator, account, reply, without_replacement, markdown):
    ...
    body += "```\n"
    if without_replacement:
        body += "blurtpypy draw -d %d -p %d -b %d -t %s -h %s -s '%s' -w\n" % (draws, participants, block["id"], trx_id, hashtype, separator)
    else:
        body += "blurtpypy draw -d %d -p %d -b %d -t %s -h %s -s '%s'\n" % (draws, participants, block["id"], trx_id, hashtype, separator)
    body += "```\n\n"
    body += "| key | value |\n"
    body += "| --- | --- |\n"
    body += "| block number | [%d](https://blurtblocks.com/b/%d#%s) |\n" % (block["id"], block["id"], trx_id)
    body += "| trx id | [%s](https://blurtblocks.com/tx/%s) |\n" % (trx_id, trx_id)
    body += "| block id | %s |\n" % block["block_id"]
    body += "| previous id | %s |\n" % block["previous"]
    body += "| hash type | %s |\n" % hashtype
    body += "| draws | %d |\n" % draws
