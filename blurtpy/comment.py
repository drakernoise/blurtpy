...
from blurtgraphenebase.py23 import py23_bytes, bytes_types, integer_types, string_types, text_type
from blurtpy.constants import BLURT_REVERSE_AUCTION_WINDOW_SECONDS_HF6, BLURT_REVERSE_AUCTION_WINDOW_SECONDS_HF20, BLURT_100_PERCENT, BLURT_1_PERCENT, BLURT_REVERSE_AUCTION_WINDOW_SECONDS_HF21
...
class Comment(BlockchainObject):
    """ Read data about a Comment/Post in the chain
        ...
        :param Blurt blockchain_instance: :class:`blurtpy.blurt.Blurt` instance to use when accessing a RPC
    ...
    def refresh(self):
        ...
        if self.blockchain.rpc.get_use_appbase():
            from blurtapi.exceptions import InvalidParameters
            try:
                if self.api == "tags":
    ...
    def curation_penalty_compensation_SBD(self):
        ...
        if self.blockchain.hardfork >= 21:
            reverse_auction_window_seconds = BLURT_REVERSE_AUCTION_WINDOW_SECONDS_HF21
        elif self.blockchain.hardfork >= 20:
            reverse_auction_window_seconds = BLURT_REVERSE_AUCTION_WINDOW_SECONDS_HF20
        else:
    ...
    def get_curation_penalty(self, vote_time=None):
        ...
        if self.blockchain.hardfork >= 21:
            reward = (elapsed_seconds / BLURT_REVERSE_AUCTION_WINDOW_SECONDS_HF21)
        elif self.blockchain.hardfork >= 20:
            reward = (elapsed_seconds / BLURT_REVERSE_AUCTION_WINDOW_SECONDS_HF20)
        else:
            reward = (elapsed_seconds / BLURT_REVERSE_AUCTION_WINDOW_SECONDS_HF6)
        if reward > 1:
            reward = 1.0
        return 1.0 - reward
    ...
    def upvote(self, weight=+100, voter=None):
        ...
        if last_payout is not None:
            if formatToTimeStamp(last_payout) > 0:
                raise VotingInvalidOnArcblurtdPost
    ...
    def reblurt(self, identifier=None, account=None):
        """ Reblurt a post
